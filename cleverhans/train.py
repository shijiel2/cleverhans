...
def train_adv(sess, loss, x_train, y_train, x_train_adv_lst=None,
          init_all=False, evaluate=None, feed=None, args=None,
          rng=None, var_list=None, fprop_args=None, optimizer=None,
          devices=None, x_batch_preprocessor=None, use_ema=False,
          ema_decay=.998, run_canary=None,
  ...
  :param loss: tensor, the loss to minimize
  :param x_train: numpy array with training inputs or tf Dataset
  :param x_train_advs: a list of numpy array with adversarial training inputs or tf Dataset
  :param y_train: numpy array with training outputs or tf Dataset
  :param init_all: (boolean) If set to true, all TF variables in the session
                   are (re)initialized, otherwise only previously
                   uninitialized variables are initialized before training.
  ...
  preprocessed_xs = []
  ys = []
  # [[placeholder x_adv1], [placeholder x_adv2], [placeholder x_adv3]]
  if x_train_adv_lst is not None:
    xs_adv_lst = [[]] * len(x_train_adv_lst)
  else:
    xs_adv_lst = None

  
  # deal with dataset_train is given
  if dataset_train is not None:
    assert x_train is None and y_train is None and x_batch_preprocessor is None
    if dataset_size is None:
      raise ValueError("You must provide a dataset size")
  ...
  for device in devices:
    with tf.device(device):
      ...
      if x_train_adv_lst is not None:
        # [x_adv1, x_adv2, x_adv3]
        x_adv_lst = [tf.placeholder(x_train.dtype, (None,) + x_train.shape[1:])] * len(x_train_adv_lst)
        for i in range(len(x_train_adv_lst)):
          xs_adv_lst[i].append(x_adv_lst[i])
      else:
        x_adv_lst = None
      ...
      loss_value = loss.fprop(x, y, x_adv_lst=x_adv_lst, **fprop_args)
  ...
  for epoch in xrange(args.nb_epochs):
    if dataset_train is not None:
      ...
      y_train_shuffled = y_train[index_shuf]
      if x_train_adv_lst is not None:
        x_train_adv_lst_shuffled = [x_train_adv[index_shuf] for x_train_adv in x_train_adv_lst]
    ...
    for batch in range(nb_batches):
      ...
      for dev_idx in xrange(num_devices):
        ...
        # feed each x_adv
        if x_train_adv_lst is not None:
          for i in range(len(x_adv_lst)):
            feed_dict[xs_adv_lst[i][dev_idx]] = x_train_adv_lst_shuffled[i][cur_start:cur_end]
